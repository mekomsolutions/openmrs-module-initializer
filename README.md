...
* (_For devs._) Introduced a pre-loading mechanism to `BaseFileLoader` that allows checksums-independent loading of transient information out of the config files before the actual metadata are loaded. The behavior of all sub classes not overriding `throwingOnPreload` to return false, is to always, as the default, throw exceptions early and intercept the actual loading of the files given the runtime setting of `initializer.startup.load = fail_on_error` was configured.
